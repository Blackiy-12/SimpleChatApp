...
{
public:
    static std::shared_ptr<IMessage> createMessage(Protocol Protocol, std::string Nickname, std::string MessageContent, uint16_t MessageID = 0x0);

    static std::shared_ptr<IMessage> createReply(Protocol Protocol, bool Result, std::string MessageContent, uint16_t MessageID = 0x0, uint16_t RefMessage = 0x00);

    static std::shared_ptr<IMessage> createMessageToConfirm(uint16_t MessageID);
    
    static std::shared_ptr<IMessage> createError(Protocol Protocol, std::string ErrorContent, uint16_t MessageID = 0x0);

    static std::shared_ptr<IMessage> createByeMessage(Protocol Protocol, uint16_t MessageID = 0x0);

    static std::shared_ptr<IMessage> translateMessage(Protocol Protocol, std::vector<uint8_t> Bytes);

private:
    static std::shared_ptr<IMessage> translateMessageUDP(std::vector<uint8_t> Bytes);
    static std::shared_ptr<IMessage> translateMessageTCP(std::vector<uint8_t> Bytes);
